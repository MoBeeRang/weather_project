         ...
         reader.onload = (event) => {
            try {
               ...
               const filteredData = jsonData
                  .filter((city) => representativeCities.some((repCity) => repCity.english_name.toLowerCase() === city.name.toLowerCase()))
                  .map((city) => {
                     // langs가 있는지 확인 후 find 실행
                     const korName = city.langs && Array.isArray(city.langs) ? city.langs.find((lang) => lang.ko)?.ko || 'Unknown' : 'Unknown'
               ...
               // 새 JSON 파일 생성
               const filteredDataBlob = new Blob([JSON.stringify(filteredData, null, 2)], { type: 'application/json' })
